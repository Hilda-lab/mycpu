...
一个从零实现的 x86 子集指令集模拟器，支持可视化调试、汇编语言编译、中断系统和内存分页。
...
- **指令集**：已实现 36+ 条指令（数据传送、算术、逻辑、移位、控制流、栈操作、中断）

### 中断系统（新增）
- **PIC 中断控制器**：支持 16 个 IRQ，中断优先级，中断屏蔽
- **PIT 定时器**：可编程定时器，支持多种工作模式
- **UART 串口**：完整的 UART 实现，支持输入/输出和中断
- **IDT 中断描述符表**：支持 256 个中断向量，中断门/陷阱门

### 内存管理（新增）
- **MMU 内存管理单元**：支持虚拟地址翻译
- **分页机制**：4KB 页面，二级页表（页目录 + 页表）
- **TLB 缓存**：64 项转换后备缓冲区
